...
@objc private protocol Aging {
    func birthday(_ person: CodableBox, reply: @escaping (CodableBox?, Error?) -> Void)
}
...
private final class AgingService: NSObject, Aging {
    func birthday(_ person: CodableBox, reply: @escaping (CodableBox?, Error?) -> Void) {
        do {
            ...
            reply(try CodableBox(Person(name: p.name, age: p.age + 1)), nil)
        } catch {
            reply(nil, error)
        }
    }
...
final class CodableBoxOverNSXPCTests: XCTestCase {
    ...
    func testRoundTripsOverARealConnection() throws {
        ...
        service.birthday(try CodableBox(Person(name: "Ada", age: 36))) { box, error in
